...
ZAP.prototype._generateResponse = function(req, returnPath, statusCode, message, userId, metadata) {
	...
	return (returnPath || []).splice(0).concat([
		...
		(((statusCode == 200) && (metadata)) ? metadata : new Buffer(0))
	]);
}

ZAP.prototype.use = function(mechanism, handler) {
	if (!mechanism) throw new Error('Invalid mechanism');
	if (!handler || (typeof handler.authenticate !== 'function')) throw new Error('Invalid mechanism handler');
	
	this._mechanisms[mechanism] = handler;
}
...
ZAP.prototype.authenticate = function(data, callback) {
	...
	var returnPath = this._captureReturnPath(data),
		...
		mech = this._mechanisms[req.mechanism];
	
	if (req && this._validateRequest(req, mech)) {
		debug('REQ: %s', JSON.stringify(req, true, 2));
		...
		if (mech) {
			mech.authenticate(req, function(err, user, message, metadata) {
				...
				if (err) {
					...
					statusCode = 200;
					userId = (user !== true) ? user : undefined;
					meta = metadata;
				} else {
					statusCode = 400;
				}
			...
			callback(new Error("No Authenticator"), res);
		}
	} else {
		var res = this._generateResponse(req, returnPath, 300, "INVALID MESSAGE");
		callback(new Error("Invalid Message"), res);
	}
}
