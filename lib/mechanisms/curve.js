...
CurveMechanism.prototype.validate = function(req) {
	if (!req.credentials || (req.credentials.length != 1)) {
		debug('Credentials should contain one entry');
		debug('Invalid credential data: %s', req.credentials ? JSON.stringify(req.credentials, true, 2) : 'undefined');		debug('Invalid credential data');
		return false;
	}
	return true;
}
